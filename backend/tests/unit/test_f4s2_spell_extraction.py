...
def test_two_observations_of_one_spell_resolve_the_same_way_in_either_order() -> None:
    ...
    It held for the two fields that decide `censored` and `effective_dom` —
    the part NORTH_STAR actually protects — but not for the ride-along
    fields, and `sqft` is the one that matters: it feeds F4-S5's `premium`,
    and a missing-sqft comp is *default-excluded* from every median. So the
    field most likely to differ between two reports of one unit was the one
    that decides whether the comp counts at all, and it was resolved by
    arrival order.

    Reachable precisely because of F4-S2: before it these two listing ids
    were separate comps and were never compared. Six real groups now take
    this path.

    **Was a strict xfail (F4-S2 review item M1); F4-S3 closed it** (PM
    ruling P1b) by giving `_completeness` the field-completeness component
    it is named for, sorted below both removal components so it can never
    censor a comp that was observed to have left the market. The marker was
    removed here as its own reason instructed."""
