...
| 5 | F0-S2 derivation graph | PIPE | F0-S3, F0-S5 | **DISPATCHED (ADR)** | blockers DONE. **CHECKPOINT: ADR drafted → owner sign-off → only then QA/impl.** ADR must also rule: Config `list`→`tuple` + hashability (see log); ⚠ register /api/derive router BEFORE create_app()'s static mount |
