...
- **Isolated-worktree dispatches (`isolation: "worktree"`) only have one shared `.venv/` — the one at the main checkout's repo root.** Its editable install (`pip install -e .`) resolves imports against the main checkout's `backend/src`, not the worktree's own copy. Prepend `PYTHONPATH=<this-worktree>/backend/src` to `pytest`/`python` invocations run inside a worktree, or the run silently tests the *wrong tree's* code. Found by the F2-S2 dev (2026-07-27).
  ...
  - **⚠ The `.pth` does NOT always win — which is worse than if it always did, and it explains a confusing class of result.** F4-S4's QA established the real mechanism (2026-07-29): `scripts/gate.py:64` does `sys.path.insert(0, <repo>/backend/src)`, and `pytest.ini` collects `scripts/tests` **before** `backend/tests`, so on a **full bare `pytest`** run from a worktree, `gate.py` is imported first and puts the **worktree's** source at `sys.path[0]`. **So a bare full run does test your branch — by accident of collection order, not by design.** A single-file or single-directory run has no such accident and loads main's source. Proven three ways: `pytest backend/tests` → **collection ERROR** (main's `shape.py` lacks the symbol under test); full bare `pytest` → no error, 1288 passed; the difference is entirely which module got imported first.
    **Two consequences.** (1) **This sharpens the standing "never run `pytest backend/tests`" rule: in a worktree it does not merely under-collect by 44 — it silently loads the WRONG SOURCE TREE.** (2) Do not reason from "the bare run passed" to "my `PYTHONPATH` was unnecessary" — you were rescued by collection order, and any change to `pytest.ini`'s order or to `gate.py`'s import removes the rescue silently. **Always set `PYTHONPATH` explicitly, and always verify the loaded module** (print `rentcomp.<module>.__file__`, or better, pin it — see the collection-time guard in row 37b).
  ...
  - **Running Playwright from a worktree — use these two doors, never a junction.** `git worktree remove --force` follows directory junctions and destroyed the main checkout's `.venv` and `e2e/node_modules` on 2026-07-28 (INCIDENT #5); agents only created those junctions because they had no other way, and these remove the reason.
    1. **Dependency resolution — but ONLY when the worktree has no `e2e/node_modules` of its own.** If it does not: set `NODE_PATH=<main checkout>/e2e/node_modules` and invoke `<main checkout>/e2e/node_modules/@playwright/test/cli.js` from the worktree's `e2e/`. Bare `npx` silently fetches a standalone Playwright where `@playwright/test` is unresolvable. (F2-S1 QA, 2026-07-28.)

      - ⚠ **AMENDED 2026-07-31 by F6-S1's QA verify, and this one is the "did not run" family in its most deceptive form yet.** If the worktree **already has** its own real `e2e/node_modules`, pointing `NODE_PATH` at main's loads **two module instances of `@playwright/test`**. `test.describe.configure` then throws `TestTypeImpl._currentSuite`, and the run reports **`Total: 0 tests in 0 files`** — ⚠ **and the error names an unrelated spec's `mode: "serial"` line, so it reads exactly like a spec defect, which it is not.** Both trees had the same version (1.44.1) and the worktree's copy was verified **not** a junction, so there is no INCIDENT #5 exposure — the failure is purely duplicate module identity. **Fix: use the worktree's OWN `node_modules` consistently whenever it has one.** Note the shape: **a zero-test run with a plausible-looking error message is indistinguishable from a suite that ran**, and it blames the wrong file. Check `Total: N tests` against what you expected to collect, every time.
    1b. **⚠ `RENTCOMP_VENV_DIR` — copying `rentcomp.exe` is NOT sufficient on its own.** Some specs also `execFileSync` the venv's **`VENV_PYTHON`** (F1-S2's seeding spec does), so a worktree with only the copied `.exe` dies with `spawnSync …\.venv\Scripts\python.exe ENOENT` and the affected tests report **"did not run"** rather than failing loudly. Set **`RENTCOMP_VENV_DIR=<main checkout>/.venv`**. Found by F3-S4's dev (2026-07-29) after door 2 alone left 4 tests unrun. **Note the failure shape: "did not run" is the same family as the silent skip — it does not look like a red test.**
  ...
  - **⚠ Port 8000 contention across parallel agents — a false green with no bad code anywhere.** `local-server.ts` spawns a server and then polls `127.0.0.1:8000`. If another agent already holds that port, the spawn's bind fails but **`waitForServer` succeeds against the other agent's build**, and the spec silently tests the wrong code while reporting green. Hit by F5-S2's dev, which waited rather than killing the other process — the right call. **This risk is created by running several Playwright-capable agents at once, so it is the PM's to manage:** stagger browser-leg dispatches, or have the spec bind an ephemeral port. Until then, **any worktree Playwright run must verify the server it reached is its own** (diff the route list, or assert on a symbol that exists only on the branch).
    ...
    - ✅ **SOLVED 2026-07-29 by F7-S1's QA — use this pattern, and migrate specs to it as you touch them.** The contention is not inherent: `__main__.py` hardcodes port 8000 with no override, but **a spec does not need the console script.** Launch `python -m uvicorn rentcomp.app:app --port <ephemeral>` — the *same* ASGI object `rentcomp` serves — and the collision is structurally gone rather than managed by staggering. Pair it with an identity check that is **neither a status code nor a guessable version string**: assert **our child process is still alive** (a lost bind kills uvicorn, so a live port plus a dead child means you are on someone else's server) **and that its startup banner names our port** — checked before the first test and again after the last. Verified clean running concurrently with three other agents. **This retires the "stagger browser-leg dispatches" instruction for any spec that adopts it.**

  - ⚠ **`mode: "serial"` is silently costing you red-state information — this is the "did not run" family again.** `playwright.config.ts` already pins `workers: 1, fullyParallel: false`, so a spec-level `test.describe.configure({mode: "serial"})` adds **nothing but "stop after the first failure"**. F7-S1's QA measured it: the same red suite reported **`1 failed, 6 did not run`** with serial mode and **`5 failed, 3 passed, 0 did not run`** without it. On a multi-AC story that means the developer fixes one red at a time and never sees the shape of the failure set — and **"did not run" reads like a pass to anyone scanning exit codes**, exactly the defect class that has now recurred five times here. Five specs still carry the marker (row 44a).

  - **⚠ The vitest leg needs `npm install` in `frontend/` first — on a worktree AND on the main checkout — or it dies in startup looking like a code failure.** F5-S2 added `vitest`+`jsdom` as devDependencies, so any checkout whose `frontend/node_modules` predates that merge will fail. Worse, **bare `npx vitest` then silently resolves vitest from the npm cache** (`_npx/<hash>/node_modules/vitest`) rather than from `frontend/node_modules`, and reports `Cannot find package 'jsdom'` — the *same* standalone-fetch trap already documented for `npx playwright`, in a new place. PM hit this on the main checkout immediately after merging F5-S2 (2026-07-28); `cd frontend && npm install` fixed it and the leg went to **5 passed**. Playwright's `beforeAll` self-heals by running `npm install`; **the vitest leg does not.** (F5-S2 QA + PM, 2026-07-28.)
